...
function institution_get_subscription_ajax() {
    // Verify user is logged in and has institution role
    if (!is_user_logged_in() || !current_user_can('institution')) {
...
function institution_add_user_ajax() {
    ...
    if ($existing_user) {
        ...
        // Generate unique IDs for the patient
        ...
        update_user_meta($user_id, 'account_type', 'patient');
        update_user_meta($user_id, 'institution_id', $institution_id);
        
        // Create patient record in the patient_details table
        global $wpdb;
        ...
        $result = attentrack_add_institution_member($institution['id'], $user_id, 'patient', $institution_id);
...
function institution_edit_details_handler() {
    ...
    // Check if user is logged in and has institution role
    if (!is_user_logged_in() || !current_user_can('institution')) {
