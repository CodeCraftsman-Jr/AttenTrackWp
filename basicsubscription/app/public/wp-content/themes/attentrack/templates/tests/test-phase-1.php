...
jQuery(document).ready(function($) {
    ...
    inputBox.on('input', function() {
        ...
        if (input.length === 1) {
            ...
            responses.push({
                ...
                correct: (currentLetter === 'X' && input === 'X') || (currentLetter !== 'X' && input !== 'X')
            });
    ...
    function submitResults() {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = '<?php echo esc_url(home_url('/test-break')); ?>';
                }
            }
        });
    }
