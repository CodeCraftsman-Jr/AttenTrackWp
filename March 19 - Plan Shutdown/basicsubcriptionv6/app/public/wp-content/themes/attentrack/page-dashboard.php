...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $chart_data = $wpdb->get_results($wpdb->prepare(
        ...
        LEFT JOIN {$wpdb->prefix}test_sessions ts ON tr.session_id = ts.id 
        WHERE tr.user_id = %d 
        ORDER BY COALESCE(ts.start_time, tr.test_date) DESC, tr.test_phase ASC
        LIMIT 15",
        $user_id
    ));
    ...
    foreach ($chart_data as $result) {
        $date = !empty($result->start_time) ? $result->start_time : $result->test_date;
        $labels[] = 'Phase ' . $result->test_phase . ' (' . date('m/d', strtotime($date)) . ')';
        $scores[] = $result->score;
