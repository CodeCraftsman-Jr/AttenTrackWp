                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                ?>
                                <tr>
                                    <td><?php echo date('F j, Y g:i a', strtotime($result->test_date)); ?></td>
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    $overall_accuracy = $wpdb->get_var($wpdb->prepare(
                                        "SELECT AVG(accuracy) FROM $table_name WHERE user_id = %d",
                                        $user_id
                                    ));
                                    ?>
                                    <div class="mt-3">
                                        <p><strong>Total Tests Taken:</strong> <?php echo $total_tests; ?></p>
                                        <p><strong>Overall Average Accuracy:</strong> <?php echo number_format($overall_accuracy, 2); ?>%</p>
                                        <?php foreach ($summary as $phase) : ?>
                                            <div class="mb-3">
                                                <h6>Phase <?php echo esc_html($phase->test_phase); ?></h6>
                                                <p>Tests: <?php echo esc_html($phase->total_tests); ?><br>
                                                   Average Score: <?php echo number_format($phase->avg_score, 1); ?><br>
                                                   Average Accuracy: <?php echo number_format($phase->avg_accuracy, 1); ?>%<br>
                                                   Average Reaction Time: <?php echo number_format($phase->avg_reaction_time, 3); ?>s</p>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $chart_data = $wpdb->get_results($wpdb->prepare(
        "SELECT test_phase, accuracy, test_date 
         FROM $table_name 
         WHERE user_id = %d 
    ...
    foreach ($chart_data as $data) {
        if (!isset($phases[$data->test_phase])) {
            $phases[$data->test_phase] = array();
        }
        $phases[$data->test_phase][] = array(
            'x' => date('Y-m-d', strtotime($data->test_date)),
            'y' => $data->accuracy
        );
    }
    ?>
