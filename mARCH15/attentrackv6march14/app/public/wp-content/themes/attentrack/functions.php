...
// Register test templates and create necessary pages
function attentrack_register_test_templates() {
    // Register templates
    add_filter('theme_page_templates', function($templates) {
        $templates['templates/test-data-insertion.php'] = 'Test Data Insertion';
        $templates['templates/test-phase-0-template.php'] = 'Test Phase 0';
        return $templates;
    });

    // Create Test Data Insertion page if it doesn't exist
    $test_data_page = get_page_by_path('test-data-insertion');
    if (!$test_data_page) {
        $page_data = array(
        ...
        if (!is_wp_error($page_id)) {
            update_post_meta($page_id, '_wp_page_template', 'templates/test-data-insertion.php');
        }
    }

    // Create Test Phase 0 page if it doesn't exist
    $test_phase_page = get_page_by_path('test-phase-0');
    if (!$test_phase_page) {
        $page_data = array(
        ...
        $page_id = wp_insert_post($page_data);
        if (!is_wp_error($page_id)) {
            update_post_meta($page_id, '_wp_page_template', 'templates/test-phase-0-template.php');
        }
    }
}

// Run template registration after theme setup
...
function attentrack_template_loader($template) {
    if (is_page()) {
        ...
        if ($page_template === 'templates/test-phase-0-template.php') {
            $new_template = locate_template('templates/test-phase-0-template.php');
            if (!empty($new_template)) {
                return $new_template;
            }
        }
        
        if ($page_template === 'templates/test-data-insertion.php') {
            $new_template = locate_template('templates/test-data-insertion.php');
            if (!empty($new_template)) {
                return $new_template;
            }
        }
...
function initialize_test_results_table() {
    global $wpdb;
...
add_action('init', 'initialize_test_results_table');
...
function save_test_results_handler() {
    try {
        ...
        if (!isset($_POST['_ajax_nonce']) || !wp_verify_nonce($_POST['_ajax_nonce'], 'attentrack_test_nonce')) {
        ...
        $data = array(
            ...
            'test_id' => isset($_POST['test_id']) ? sanitize_text_field($_POST['test_id']) : uniqid('test_'),
            'test_type' => isset($_POST['test_type']) ? sanitize_text_field($_POST['test_type']) : 'phase_0',
            ...
            'total_letters' => isset($_POST['total_letters']) ? intval($_POST['total_letters']) : 0,
            'p_letters' => isset($_POST['p_letters']) ? intval($_POST['p_letters']) : 0,
            'test_date' => current_time('mysql')
        );

        if (isset($_POST['responses'])) {
            $responses = stripslashes($_POST['responses']);
            $decoded = json_decode($responses, true);
            if (json_last_error() === JSON_ERROR_NONE) {
                $data['responses'] = wp_json_encode($decoded);
            } else {
                $data['responses'] = '[]';
            }
        } else {
        ...
        error_log('Error in save_test_results_handler: ' . $e->getMessage());
...
remove_action('wp_ajax_save_test_results', 'save_test_results_handler');
remove_action('wp_ajax_nopriv_save_test_results', 'save_test_results_handler');
...
// Register scripts and localize AJAX URL
function attentrack_register_ajax_scripts() {
...
add_action('wp_enqueue_scripts', 'attentrack_register_ajax_scripts');

// Enqueue scripts and styles for the theme
function attentrack_enqueue_scripts() {
    // Only load test scripts on the test page
    if (is_page('test1phase1')) {
    ...
    // Load scripts for test phase 0
    if (is_page('test-phase-0')) {
        wp_enqueue_script('jquery');
    ...
    wp_enqueue_script('attentrack-scripts', get_template_directory_uri() . '/js/scripts.js', array('jquery'), null, true);
    
    // Enqueue Chart.js
    wp_enqueue_script('chartjs', 'https://cdn.jsdelivr.net/npm/chart.js', array(), '3.7.0', true);
    
    // Enqueue auth.js script
    wp_enqueue_script(
        'attentrack-auth',
...
function attentrack_create_tables() {
    ...
    // Drop existing test results table to recreate with correct schema
    $wpdb->query("DROP TABLE IF EXISTS $test_results_table");

    // Create test_results table with direct SQL
    $wpdb->query("CREATE TABLE $test_results_table (
        id bigint(20) NOT NULL AUTO_INCREMENT,
        user_id bigint(20) NOT NULL,
        test_id varchar(50) NOT NULL,
        test_type varchar(50) NOT NULL,
        test_phase int(11) NOT NULL DEFAULT 0,
        score float NOT NULL DEFAULT 0,
        accuracy float NOT NULL DEFAULT 0,
        reaction_time float NOT NULL DEFAULT 0,
        missed_responses int(11) NOT NULL DEFAULT 0,
        ...
        total_letters int(11) NOT NULL DEFAULT 0,
        p_letters int(11) NOT NULL DEFAULT 0,
        switch_time float DEFAULT NULL,
        completion_time float DEFAULT NULL,
        errors int(11) DEFAULT NULL,
        task1_accuracy float DEFAULT NULL,
        task2_accuracy float DEFAULT NULL,
        dual_task_cost float DEFAULT NULL,
...
// Run table creation now to ensure proper schema
...
function validate_test_results($data) {
    ...
    // Validate test type
    $valid_test_types = array('selective_attention', 'selective_attention_extended', 'alternative_attention', 'divided_attention', 'phase_0');
...
// Enhanced debug logging for test results
function log_test_data($data, $prefix = '') {
    error_log("\n=== $prefix Test Data Debug Info ===\n");
    error_log(print_r($data, true));
}

// Initialize test results table with proper encoding
function initialize_test_results_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'test_results';
    ...
    error_log("\nTest results table exists: " . ($table_exists ? 'Yes' : 'No'));
    if (!$table_exists) {
        error_log("\nFailed to create table. Last error: " . $wpdb->last_error);
    }
}

// AJAX handler to save test results with enhanced debugging
function save_test_results_handler() {
    global $wpdb;
    
    error_log("\n=== Starting save_test_results_handler ===\n");
    ...
    try {
        ...
        if (!$saved_data) {
            throw new Exception('Failed to verify saved data');
        }
        ...
        error_log("\nError in save_test_results_handler: " . $e->getMessage());
...
// AJAX handler to get unique test ID
function get_unique_test_id_handler() {
    error_log("\n=== Starting get_unique_test_id_handler ===\n");
    ...
    if (empty($nonce) || !wp_verify_nonce($nonce, 'attentrack_test_nonce')) {
        error_log("\nNonce verification failed. Received nonce: " . $nonce);
    ...
    // Generate unique test ID using user ID and timestamp
    $test_id = sprintf('test_%d_%s', $current_user->ID, time());
    error_log("\nGenerated test ID: " . $test_id);
    
    // Store test ID in user meta for reference
    update_user_meta($current_user->ID, 'current_test_id', $test_id);
    error_log("\nStored test ID in user meta");
    
    wp_send_json_success($test_id);
}

// Remove any duplicate handlers
remove_action('wp_ajax_get_unique_test_id', 'get_unique_test_id');
remove_action('wp_ajax_get_unique_test_id', 'get_unique_test_id_handler');
remove_action('wp_ajax_nopriv_get_unique_test_id', 'get_unique_test_id_handler');
...
function attentrack_flush_rewrite_rules() {
    attentrack_register_test_templates();
    flush_rewrite_rules();
}
register_activation_hook(__FILE__, 'attentrack_flush_rewrite_rules');

// Also flush rules when saving permalinks
function attentrack_flush_rules_on_save() {
    if (isset($_POST['permalink_structure']) || isset($_POST['category_base'])) {
        attentrack_flush_rewrite_rules();
    }
}
add_action('admin_init', 'attentrack_flush_rules_on_save');
