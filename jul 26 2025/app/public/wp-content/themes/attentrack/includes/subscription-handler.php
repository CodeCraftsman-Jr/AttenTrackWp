...
function handle_successful_payment($payment_id, $order_id, $signature) {
    ...
    write_log("Deactivated existing subscriptions for user: {$user_id}");
    ...
    if ($result === false) {
        ...
        write_log("Subscription data that failed: " . json_encode($subscription_data), 'error');

        // Try to reactivate existing subscription if insert fails
        $existing_sub = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}attentrack_subscriptions
            WHERE user_id = %d AND plan_name = %s
            ORDER BY created_at DESC LIMIT 1",
            $user_id, $plan_type
        ));

        if ($existing_sub) {
            write_log("Reactivating existing subscription ID: {$existing_sub->id}");
            $wpdb->update(
                $wpdb->prefix . 'attentrack_subscriptions',
                array('status' => 'active', 'payment_id' => $payment_id, 'order_id' => $order_id),
                array('id' => $existing_sub->id)
            );
        }
    } else {
        ...
        // Ensure no other subscriptions are active for this user
        $wpdb->query($wpdb->prepare(
            "UPDATE {$wpdb->prefix}attentrack_subscriptions
            SET status = 'inactive'
            WHERE user_id = %d AND id != %d AND status = 'active'",
            $user_id, $subscription_id
        ));

        write_log("Ensured only subscription ID {$subscription_id} is active for user {$user_id}");
    }
