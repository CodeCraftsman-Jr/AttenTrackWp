...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        if (window.location.pathname.includes('/signin') || window.location.pathname.includes('/signup')) {
            console.log('On signin/signup page, checking for redirect result');
            ...
            firebase.auth().getRedirectResult()
                ...
                .catch((error) => {
                    ...
                    if (error.code !== 'auth/null-credential') {
                        ...
                        console.log('Normal page load (no redirect in progress)');
                    }
                });
        }
        ...
        window.auth.onAuthStateChanged((user) => {
            ...
            // Check if user is logged in via WordPress (server-side authentication)
            const isWordPressLoggedIn = document.body.classList.contains('logged-in');

            // If user is logged in via WordPress but not Firebase, don't redirect
            if (isWordPressLoggedIn && !user) {
                console.log('User is logged in via WordPress but not Firebase - no redirect needed');
                return;
            }
            ...
            if (user) {
                ...
                // User is signed out - only protect specific authenticated pages
                // Allow public pages like home, about, contact to be accessed without authentication
                const protectedPages = ['/dashboard', '/selection-page', '/institution-dashboard', '/patient-dashboard'];
                const isProtectedPage = protectedPages.some(page => currentPath.includes(page));

                // Only redirect if on a protected page and not already on signin/signup
                if (isProtectedPage) {
                    console.log('User is not signed in, redirecting from protected page to signin');
                    // Redirect to signin
                    const signinLink = document.querySelector('a[href*="signin"]')?.href || '/signin';
                    window.location.href = signinLink;
                } else {
                    console.log('User is not signed in, but on public page - no redirect needed');
                }
            }
        });
