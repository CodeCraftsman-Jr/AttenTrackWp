<?php
/**
 * Template Name: Selective and Sustained Attention Test Part 3
 */
 ...
 <body>
     ...
     <div id="test-area" class="position-relative">
         ...
         <script>
             console.log('Test Phase 3 script starting...');
             ...
             function showModal() {
                 ...
                 modalBody.innerHTML = `
                     Results:<br>
                     Total 'p' letters found: ${correctCount}<br>
                     Correct responses: ${correctCount}<br>
                     Incorrect responses: ${incorrectCount}<br>
                     Missed responses: ${missedCount}<br>
                     Total letters shown: ${responses.length}
                 `;
                 modal.style.display = "block";
                 
                 // Calculate metrics for database
                 let totalResponses = responses.length;
                 let correctResponses = correctCount;
                 let accuracy = (correctResponses / totalResponses) * 100;
                 let avgReactionTime = responses.reduce((sum, response) => sum + response.reactionTime, 0) / totalResponses;
                 let falseAlarms = incorrectCount;
                 ...
                 jQuery.ajax({
                     ...
                     success: function(response) {
                         if (response.success) {
                             ...
                             window.location.href = '<?php echo get_permalink(get_page_by_path("selective-and-sustained-attention-test-part-4")); ?>?test_id=' + testID;
                         } else {
