<?php
/**
 * Template Name: Selective and Sustained Attention Test Part 4
 */
 ...
 <body>
     ...
     <div id="gameOverModal" class="modal">
         <div class="modal-content">
             ...
             <button class="modal-button" id="continueButton">View Results</button>
         </div>
     </div>

     <script>
         console.log('Test Phase 4 script starting...');
         ...
         // After game ends, redirect to dashboard instead of next phase
         continueButton.addEventListener('click', function() {
             jQuery.ajax({
                ...
                data: {
                    ...
                    test_phase: 4,
                    score: correctCount,
                    accuracy: (correctCount / (correctCount + incorrectCount + missedCount)) * 100,
                    reaction_time: (responses.reduce((sum, response) => sum + response.reactionTime, 0) / responses.length) / 1000,
                    missed_responses: missedCount,
                    false_alarms: incorrectCount,
                    responses: JSON.stringify(responses)
                },
                success: function(response) {
                    if (response.success) {
                        console.log('Results saved successfully:', response.data);
                        // Redirect to selection page after completing all parts
                        window.location.href = '<?php echo home_url("/selection-page"); ?>';
                    } else {
