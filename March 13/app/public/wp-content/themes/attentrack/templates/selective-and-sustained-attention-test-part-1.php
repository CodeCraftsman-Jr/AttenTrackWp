...
// Log for debugging
error_log('Test ID from URL: ' . print_r($test_id, true));
error_log('GET parameters: ' . print_r($_GET, true));

if (empty($test_id)) {
    // If no test ID, generate one
    $current_user = wp_get_current_user();
    $test_id = generate_unique_test_id($current_user->ID);
    error_log('Generated new test ID: ' . $test_id);
}
...
<head>
    ...
    <?php
        ...
        wp_localize_script('jquery', 'attentrack_ajax', array(
            ...
            'next_test_url' => get_permalink(get_page_by_path('selective-and-sustained-attention-test-part-2'))
        ));
    ?>
    <style>
        body {
...
<body>
    ...
    <div id="test-id"><?php echo 'Test ID: ' . $test_id; ?></div>
    <div id="letter"></div>
    ...
    <script>
        ...
        function updateTimer() {
            ...
            if (timeLeft <= 0 || pCount >= REQUIRED_P_COUNT) {
                gameEnded = true;
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href = attentrack_ajax.next_test_url;
                    } else {
