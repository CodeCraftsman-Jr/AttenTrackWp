...
function handleSuccessfulAuth(user) {
    ...
    // First verify we have the ajax_url
    if (typeof window.attentrack_ajax === 'undefined' || !window.attentrack_ajax.ajax_url) {
        console.error('WordPress AJAX URL not found:', window.attentrack_ajax);
        showMessage('Error: WordPress AJAX URL not configured', true);
        return;
    }

    const formData = new URLSearchParams(data);

    fetch(window.attentrack_ajax.ajax_url, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            // Use WordPress-provided URL or fallback
