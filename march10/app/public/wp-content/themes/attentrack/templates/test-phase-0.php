...
<head>
    ...
    <title>Test Phase 0 - Selective Attention Test</title>
    <style>
        body {
...
<body <?php body_class(); ?>>
    ...
    <input type="text" id="inputBox" maxlength="1" autocomplete="off" placeholder="Type Here">

    <button id="startButton" onclick="startGame()">Start Test</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!window.attentrack_ajax) {
                console.error('AJAX configuration not found. Make sure the script is properly localized.');
                return;
            }
            ...
            window.startGame = function() {
                if (!gameStarted) {
                    ...
                    // Update display
                    totalLettersElement.textContent = '0';
                    pLettersElement.textContent = '0';
                    correctCountElement.textContent = '0';
                    unattemptedCountElement.textContent = '0';
            ...
            function saveResults() {
                ...
                let score = accuracy * (1 - avgReactionTime/2000); // Simple scoring formula
                
                // Save results using the common function
                window.attentrack.saveTestResults(
                    'phase0', // Simple test ID since we now use user_id in database
                    0, // phase 0
                    score,
                    accuracy,
                    avgReactionTime/1000, // Convert to seconds
                    missedCount,
                    falseAlarms,
                    responses
                ).then(response => {
                    if (response.success) {
                        console.log('Successfully saved test results:', response.data);
                    } else {
                        console.error('Failed to save test results:', response.data?.message);
                    }
                }).catch(error => {
            ...
            function showModal() {
                    ...
                    <div class="results-summary">
                        ...
                        <table class="results-table">
                            ...
                            <tr>
                                ...
                                <td>${(avgReactionTime/1000).toFixed(3)}s</td>
                            </tr>
                            <tr>
                ...
                // Save results after showing modal
                saveResults();
